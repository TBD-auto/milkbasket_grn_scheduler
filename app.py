#!/usr/bin/env python3
"""
Milkbasket GRN Scheduler - Runs workflows every 3 hours and logs to Google Sheets
"""
...
class MilkbasketAutomation:
    ...
    def send_email_notification(self, summary_data: dict):
        ...
        try:
            ...
            # Create email body
            subject = f"Milkbasket Automation Workflow Summary"
            
            # Format the email body as specified
            body_lines = [
                "Milkbasket Automation Workflow Summary",
                "Workflow Time: {} to {}".format(
                    summary_data.get('start_time', ''),
                    summary_data.get('end_time', '')
                ),
                "",
                "Duration: {} minutes".format(summary_data.get('duration_minutes', '0')),
                "",
                "Status: {}".format(summary_data.get('status', 'Unknown')),
                "",
                "📧 Mail to Drive Workflow",
                "Days Back Parameter: {} days".format(summary_data.get('mail_days_back', 7)),
                "Number of Mails Checked: {}".format(summary_data.get('mail_emails_checked', 0)),
                "Number of Attachments Found: {}".format(summary_data.get('mail_attachments_found', 0)),
                "Number of Attachments Uploaded: {}".format(summary_data.get('mail_attachments_uploaded', 0)),
                "Number of Attachments Skipped: {}".format(summary_data.get('mail_attachments_skipped', 0)),
                "Failed to Upload: {}".format(summary_data.get('mail_upload_failed', 0)),
                "",
                "📊 Drive to Sheet Workflow",
                "Days Back Parameter: {} days".format(summary_data.get('sheet_days_back', 7)),
                "Number of Files Found: {}".format(summary_data.get('drive_files_found', 0)),
                "Number of Files Processed: {}".format(summary_data.get('drive_files_processed', 0)),
                "Number of Files Skipped: {}".format(summary_data.get('drive_files_skipped', 0)),
                "Number of Files Failed to Process: {}".format(summary_data.get('drive_files_failed', 0)),
                "Duplicate Records Removed: {}".format(summary_data.get('duplicate_records_removed', 0)),
                "",
                "=" * 50,
                "This is an automated report from Milkbasket Automation Scheduler.",
                ""
            ]
    ...
    def process_drive_to_sheet_workflow(self, config: dict, skip_existing: bool = True) -> Dict:
        ...
        try:
            ...
            if skip_existing:
                ...
                self.log(f"Skipping {len(existing_file_ids)} already processed files", "INFO")
            ...
            if skip_existing:
                ...
                pdf_files = [f for f in pdf_files if f['id'] not in existing_file_ids]
                stats['skipped_pdfs'] = original_count - len(pdf_files)
                stats['duplicate_records_removed'] = stats['skipped_pdfs']  # Assuming each skipped file would have produced at least one row
            ...
            self.log(f"[STATS] Duplicate records removed: {stats['duplicate_records_removed']}")
    ...
    def run_scheduled_workflow(self):
        ...
        try:
            ...
            start_time_str = overall_start.strftime('%Y-%m-%d %H:%M:%S')
            ...
            end_time_str = overall_end.strftime('%Y-%m-%d %H:%M:%S')
            ...
            # Determine status
            status = "Completed Successfully" if (mail_stats.get('success', False) and sheet_stats['processed_pdfs'] > 0) else "Partial Success" if (mail_stats.get('success', False) or sheet_stats['processed_pdfs'] > 0) else "Failed"
            ...
            summary_data = {
                ...
                'duplicate_records_removed': sheet_stats.get('duplicate_records_removed', 0)
            }
            ...
            self.log(f"Total Duration: {duration_minutes:.2f} minutes")
...
def main():
    ...
    if summary:
        print("\nWorkflow Summary:")
        print(f"  Days Back: {summary['mail_days_back']}")
